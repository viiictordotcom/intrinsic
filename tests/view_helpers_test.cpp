...
TEST_CASE("view_add ticker type helpers switch and expose correct schemas")
{
    ...
    REQUIRE_EQ(views::normalize_add_ticker_type(4), 4);
    ...
    REQUIRE_EQ(views::next_add_ticker_type(1), 4);
    REQUIRE_EQ(views::next_add_ticker_type(4), 2);
    ...
    const auto& fields1b = views::add_fields_for_type(4);
    ...
    REQUIRE_EQ(fields1b.front().key, views::FieldKey::Ticker);
    ...
    REQUIRE_EQ(fields1b[4].key, views::FieldKey::TotalAssets);
    REQUIRE_EQ(fields1b[6].key, views::FieldKey::TotalLiabilities);
...
TEST_CASE("view layout helpers switch hint rows on narrow terminals")
{
    ...
    REQUIRE(views::ticker_use_three_row_help_layout(24));
    REQUIRE(!views::ticker_use_three_row_help_layout(120));
}
