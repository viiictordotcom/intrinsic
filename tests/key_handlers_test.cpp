...
TEST_CASE("key_add > submits two periods in one add form")
{
    test::AppSandbox sandbox;
    ...
    REQUIRE(views::handle_key_add(sandbox.app, '>'));
    REQUIRE_EQ(sandbox.app.add.value_columns, 2);
    REQUIRE_EQ(sandbox.app.add.column, 0);
    REQUIRE_EQ(sandbox.app.add.buffers_extra[0], std::string("MSFT"));

    REQUIRE(views::handle_key_add(sandbox.app, KEY_DOWN));
    REQUIRE(views::handle_key_add(sandbox.app, KEY_RIGHT));
    REQUIRE_EQ(sandbox.app.add.column, 1);
    for (char c : std::string("2025-y")) {
        REQUIRE(views::handle_key_add(sandbox.app, c));
    }
    REQUIRE_EQ(sandbox.app.add.buffers_extra[1], std::string("2025-Y"));

    for (std::size_t i = 0; i < sandbox.app.add.buffers_extra[1].size(); ++i) {
        REQUIRE(views::handle_key_add(sandbox.app, KEY_LEFT));
    }
    REQUIRE(views::handle_key_add(sandbox.app, KEY_LEFT));
    REQUIRE_EQ(sandbox.app.add.column, 0);
    for (char c : std::string("2024-y")) {
    ...
    const auto rows = sandbox.database.get_finances("MSFT", &err);
    REQUIRE(err.empty());
    REQUIRE_EQ(rows.size(), std::size_t{2});
    REQUIRE(views::add_find_period_index(rows, "2024-Y") >= 0);
    REQUIRE(views::add_find_period_index(rows, "2025-Y") >= 0);
}

TEST_CASE("key_add confirmation accepts enter as yes")
{
    test::AppSandbox sandbox;
...
TEST_CASE("key_add < collapses and clears extra period column")
{
    test::AppSandbox sandbox;
    ...
    REQUIRE_EQ(sandbox.app.add.value_columns, 1);
    REQUIRE_EQ(sandbox.app.add.column, 0);
    REQUIRE(sandbox.app.add.buffers_extra[0].empty());
...
TEST_CASE("key_add two-column arrows move cursor first and switch columns at edges")
{
    ...
    REQUIRE_EQ(sandbox.app.add.column, 0);
    REQUIRE_EQ(sandbox.app.add.cursor, 6);

    REQUIRE(views::handle_key_add(sandbox.app, KEY_RIGHT));
    REQUIRE_EQ(sandbox.app.add.column, 1);
    REQUIRE_EQ(sandbox.app.add.cursor, 0);
    ...
    REQUIRE(views::handle_key_add(sandbox.app, KEY_LEFT));
    REQUIRE_EQ(sandbox.app.add.column, 1);
    REQUIRE_EQ(sandbox.app.add.cursor, 5);
    ...
    REQUIRE(views::handle_key_add(sandbox.app, KEY_RIGHT));
    REQUIRE_EQ(sandbox.app.add.column, 0);
    REQUIRE_EQ(sandbox.app.add.cursor, 1);
...
TEST_CASE("key_add type1b derives non-current fields from totals")
{
    test::AppSandbox sandbox;
    ...
    REQUIRE(views::handle_key_add(sandbox.app, ' '));
    REQUIRE_EQ(sandbox.app.add.ticker_type, 4);

    for (char c : std::string("totl")) {
        REQUIRE(views::handle_key_add(sandbox.app, c));
    }
    REQUIRE_EQ(sandbox.app.add.buffers[0], std::string("TOTL"));
    ...
    REQUIRE_EQ(rows.front().net_income, std::optional<std::int64_t>{140});
    REQUIRE_EQ(rows.front().eps, std::optional<double>{3.2});
    REQUIRE_EQ(rows.front().cash_flow_from_operations,
               std::optional<std::int64_t>{190});
    REQUIRE_EQ(rows.front().cash_flow_from_investing,
               std::optional<std::int64_t>{80});
    REQUIRE_EQ(rows.front().cash_flow_from_financing,
               std::optional<std::int64_t>{60});
    REQUIRE_EQ(rows.front().total_assets, std::nullopt);
    REQUIRE_EQ(rows.front().total_liabilities, std::nullopt);

    const auto db_type = sandbox.database.get_ticker_type("TOTL", &err);
    REQUIRE(err.empty());
    REQUIRE(db_type.has_value());
    REQUIRE_EQ(*db_type, 1);
}
...
TEST_CASE("key_add edit mode disables two-column shortcuts and only edits current period")
{
    ...
    sandbox.add_finance("MSFT", "2025-Y", test::standard_payload(200, 20, 2.0));
    ...
    const int edit_idx = views::add_find_period_index(rows, "2024-Y");
    REQUIRE(edit_idx >= 0);
    views::open_add_prefilled_from_ticker(
        sandbox.app, rows[static_cast<std::size_t>(edit_idx)]);
    REQUIRE_EQ(sandbox.app.current, views::ViewId::Add);
    REQUIRE_EQ(sandbox.app.add.mode, AddMode::EditFromTicker);
    REQUIRE_EQ(sandbox.app.add.value_columns, 1);
    ...
    const auto updated_rows = sandbox.database.get_finances("MSFT", &err);
    REQUIRE(err.empty());
    REQUIRE_EQ(updated_rows.size(), std::size_t{2});

    const int idx_2024 = views::add_find_period_index(updated_rows, "2024-Y");
    const int idx_2025 = views::add_find_period_index(updated_rows, "2025-Y");
    REQUIRE(idx_2024 >= 0);
    REQUIRE(idx_2025 >= 0);
    REQUIRE(views::add_find_period_index(updated_rows, "2026-Y") < 0);

    REQUIRE_EQ(updated_rows[static_cast<std::size_t>(idx_2024)].revenue,
               std::optional<std::int64_t>{999});
    REQUIRE_EQ(updated_rows[static_cast<std::size_t>(idx_2025)].revenue,
               std::optional<std::int64_t>{200});
}
...
TEST_CASE("key_add locked type1 ticker allows switching 1a and 1b")
{
    test::AppSandbox sandbox;
    sandbox.add_finance("AAPL", "2024-Y");
    ...
    REQUIRE_EQ(sandbox.app.add.buffers[0], std::string("AAPL"));
    REQUIRE_EQ(sandbox.app.add.ticker_type, 1);
    REQUIRE(sandbox.app.add.ticker_type_locked);
...
TEST_CASE("key_settings toggles values persists settings and arms nuke")
{
    ...
    const auto old_color_mode = sandbox.app.settings.color_mode;
    ...
    REQUIRE(views::handle_key_settings(sandbox.app, 'B'));
    REQUIRE(sandbox.app.settings.color_mode != old_color_mode);
    REQUIRE_EQ(sandbox.app.settings.color_mode, ColorMode::White);

    REQUIRE(views::handle_key_settings(sandbox.app, 'B'));
    REQUIRE_EQ(sandbox.app.settings.color_mode, ColorMode::Black);

    REQUIRE(views::handle_key_settings(sandbox.app, 'B'));
    REQUIRE_EQ(sandbox.app.settings.color_mode, ColorMode::Default);
...
TEST_CASE("key_settings and key_help escape routes back home")
{
    test::AppSandbox sandbox;

    sandbox.app.current = views::ViewId::Settings;
    REQUIRE(views::handle_key_settings(sandbox.app, 27));
    REQUIRE_EQ(sandbox.app.current, views::ViewId::Home);

    sandbox.app.current = views::ViewId::Help;
