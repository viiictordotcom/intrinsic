...
- App data/settings are persisted in the WSL Linux filesystem.
...
- `enter`: open selected ticker (in search mode: submit query -> open selection)
...
- `space`: switch add form type (`t1a` / `t1b` / `t2` / `t3`) for new tickers
- `>` / `<`: switch value columns (`2 cols` / `1 col`)
...
- `T`: toggle TTM mode
- `B`: cycle color mode (`default` -> `white` -> `black`)
...
- `type 1` (default): current/general company structure (balance + income + cash flow), with add variants `t1a` (enter `NCA`/`NCL` -> `TA`/`TL` derived) and `t1b` (enter `TA`/`TL` -> `NCA`/`NCL` derived).
...
- If `wished per` is empty/zero/invalid, `P needed` and `NI needed` render as `--`.
...
Type labels below match ticker view metric labels.
