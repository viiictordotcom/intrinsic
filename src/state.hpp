...
struct AddState {
    ...
    std::vector<std::string> buffers_extra;
    std::vector<OptValue> values;
    std::vector<OptValue> values_extra;
    ...
    void reset(int field_count)
    {
        ...
        buffers_extra.assign(field_count, std::string{});
        values.assign(field_count, std::nullopt);
        values_extra.assign(field_count, std::nullopt);
