...
inline constexpr std::string_view kTickerHelpRowActions =
    "x: delete   e: edit   c: copy";
inline constexpr std::string_view kTickerHelpRowWide =
    "h/-/esc: home   ?: help   s: settings   q: quit";
inline constexpr std::string_view kTickerHelpRowNarrow =
    "h/-/esc: home   ?: help";
inline constexpr std::string_view kTickerHelpRowNarrowBottom =
    "s: settings   q: quit";

inline bool ticker_use_three_row_help_layout(int term_cols)
{
    if (term_cols <= 0) return false;
    return term_cols < static_cast<int>(kTickerHelpRowWide.size()) + 1;
}

inline void render_ticker_help_rows(int help_lines, int term_cols)
{
    if (help_lines < 2) return;

    const bool three_rows = ticker_use_three_row_help_layout(term_cols);
    auto print_hint = [&](int y, std::string_view text) {
        if (y < 0 || y >= LINES) return;
        const int max_width = std::max(0, term_cols - 1);
        const int shown = std::min(max_width, static_cast<int>(text.size()));
        mvprintw(y, 0, "%.*s", shown, text.data());
    };

    attron(A_DIM);
    if (three_rows && help_lines >= 3) {
        const int y0 = std::max(0, LINES - help_lines);
        const int start_y = (help_lines >= 4) ? (y0 + 1) : y0;
        print_hint(start_y + 0, kTickerHelpRowActions);
        print_hint(start_y + 1, kTickerHelpRowNarrow);
        print_hint(start_y + 2, kTickerHelpRowNarrowBottom);
    }
    else {
        const int y0 = std::max(0, LINES - help_lines);
        const int start_y = (help_lines >= 4) ? (y0 + 2) : y0;
        print_hint(start_y + 0, kTickerHelpRowActions);
        print_hint(start_y + 1, three_rows ? kTickerHelpRowNarrow
                                            : kTickerHelpRowWide);
    }
    attroff(A_DIM);
}

inline void render_blinking_input_caret(const AppState& app, int y, int x)
{
    curs_set(0);
...
inline std::string format_shares_opt(std::optional<double> v,
                                     const char* na_value = kNaValue)
{
    if (!v.has_value() || !std::isfinite(*v)) return na_value;
    ...
    constexpr std::int64_t kOneMillion = 1000000;
    const auto rounded = static_cast<std::int64_t>(std::llround(*v));
    if (rounded >= kOneMillion || rounded <= -kOneMillion) {
        const auto rounded_millions = rounded / kOneMillion;
        return format_f64_integer_opt(
                   std::optional<double>(static_cast<double>(rounded_millions)),
                   na_value) +
               "M";
    }

    return format_f64_integer_opt(std::optional<double>(static_cast<double>(rounded)),
                                  na_value);
}
...
inline std::optional<std::int64_t> derive_underwriting_expenses(
...
{
    if (!total_expenses.has_value() || !claims_incurred.has_value()) {
        return fallback;
    }
    return *total_expenses - *claims_incurred - interest_expenses.value_or(0);
}

inline std::optional<std::int64_t>
derived_underwriting_expenses_for_row(const db::Database::FinanceRow& row)
{
    return derive_underwriting_expenses(row.total_expenses,
                                        row.claims_incurred,
                                         ...
                                         const db::Database::FinanceRow& row)
{
    ...
    if (view.ticker_type == 3) {
        append_clipboard_i64(out, "total assets", row.total_assets);
        append_clipboard_i64(out, "total liabilities", row.total_liabilities);
        append_clipboard_i64(out, "insurance reserves", row.insurance_reserves);
        append_clipboard_i64(out, "total debt", row.total_debt);
        append_clipboard_i64(out, "earned premiums", row.earned_premiums);
        append_clipboard_i64(out, "claims incurred", row.claims_incurred);
        append_clipboard_i64(out, "interest expenses", row.interest_expenses);
        append_clipboard_i64(out, "total expenses", row.total_expenses);
        const auto underwriting_expenses =
            derived_underwriting_expenses_for_row(row);
        append_clipboard_i64(
            out, "underwriting expenses", underwriting_expenses);
        append_clipboard_i64(out, "net income", row.net_income);
        append_clipboard_f64(out, "eps", row.eps);
        ...
        const auto premiums_d = to_f64(row.earned_premiums);
        const auto claims_d = to_f64(row.claims_incurred);
        const auto expenses_d = to_f64(underwriting_expenses);
        const auto debt_d = to_f64(row.total_debt);
        const auto underwriting_profit_d = to_f64(underwriting_profit);
        const auto loss_ratio = div_opt_nonzero(claims_d, premiums_d);
        const auto expense_ratio = div_opt_nonzero(expenses_d, premiums_d);
        ...
        append_clipboard_i64(out, "equity", equity);
        append_clipboard_i64(out, "underwriting profit", underwriting_profit);
        append_clipboard_f64(out, "shares approx", shares_outstanding);
        append_clipboard_f64(out, "book value per share", book_value_per_share);
        append_clipboard_f64(out, "loss ratio", loss_ratio);
        append_clipboard_f64(out, "expense ratio", expense_ratio);
        append_clipboard_f64(out, "combined ratio", combined_ratio);
        append_clipboard_f64(
            out, "underwriting margin", div_opt_nonzero(underwriting_profit_d, premiums_d));
        append_clipboard_f64(out, "roe", div_opt_nonzero(net_income_d, equity_d));
        append_clipboard_f64(
            out, "reserves / equity", div_opt_nonzero(reserves_d, equity_d));
        append_clipboard_f64(
            out, "debt / equity", div_opt_nonzero(debt_d, equity_d));
        append_clipboard_f64(out, "p / bv", p_bv);
        append_clipboard_f64(out, "p / e", p_e);
        return out.str();
    }
                             ...
                             int body_height)
{
    ...
    const short pair = color_pair_for_change_text(change_text);
                                ...
                                const std::string& period)
{
    ...
    const auto underwriting_expenses = derived_underwriting_expenses_for_row(row);
    ...
    const auto prev_underwriting_profit =
        previous_row
            ? sub_i64(sub_i64(previous_row->earned_premiums,
    ...
    const auto loss_ratio = div_opt_nonzero(claims_incurred_d, earned_premiums_d);
    ...
    const auto p_bv = div_opt_nonzero(ratio_price, book_value_per_share);
    const auto p_e = div_opt_nonzero(ratio_price, eps_for_derived);
    const auto prev_p_bv = div_opt_nonzero(ratio_price, prev_book_value_per_share);
    ...
    const std::vector<Metric> balance_box = {
        ...
        {"Debt / E",
         with_change(format_ratio_opt(debt_to_equity),
                     ratio_percent_change(debt_to_equity, prev_debt_to_equity))},
        {"Shs~",
         with_change(format_shares_opt(shares_outstanding),
    ...
    const std::vector<Metric> income_box = {
        {"Premiums",
         with_change(format_i64_opt(row.earned_premiums),
                     percent_change(earned_premiums_d, prev_earned_premiums_d))},
        {"Claims",
         with_change(format_i64_opt(row.claims_incurred),
                     percent_change(claims_incurred_d, prev_claims_incurred_d))},
        {"Interests",
         with_change(format_i64_opt(row.interest_expenses),
    ...
    const std::vector<Metric> ratios_box = {
        ...
        {"Comb%",
         with_change(format_f64_opt(combined_ratio, true),
        ...
        {"ROE",
         with_change(format_f64_opt(roe, true), ratio_percent_change(roe, prev_roe))},
    };
