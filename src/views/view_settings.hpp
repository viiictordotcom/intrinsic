...
{
    if (mode == ColorMode::White) return "white";
    if (mode == ColorMode::Black) return "black";
    return "default";
}
...
inline void render_settings(const AppState& app)
{
    ...
    if (LINES > 2) {
        attron(A_DIM);
        mvprintw(2, 0, "use keys (H/S/O/T/B/U/N):");
        attroff(A_DIM);
    }
    ...
    if (LINES > y)
        mvprintw(y,
                 2,
                 "B  color_mode: %s",
                 color_mode_label(app.settings.color_mode));
    y += 2;
    if (LINES > y) {
...
inline bool handle_key_settings(AppState& app, int ch)
{
    ...
    if (ch == 'B') {
        if (app.settings.color_mode == ColorMode::Default) {
            app.settings.color_mode = ColorMode::White;
        }
        else if (app.settings.color_mode == ColorMode::White) {
            app.settings.color_mode = ColorMode::Black;
        }
        else {
            app.settings.color_mode = ColorMode::Default;
        }
        apply_settings_changed(app);
        return true;
    }
