...
inline constexpr std::string_view kHomeHelpMainRowWide =
    "a: add   q: quit   s: settings   ?: help";
inline constexpr std::string_view kHomeHelpMainRowNarrowTop =
    "a: add   q: quit   s: settings";
inline constexpr std::string_view kHomeHelpMainRowNarrowMiddle =
    "space: search   ?: help";
inline constexpr std::string_view kHomeHelpMainRowNarrowBottom =
    "p: mark portfolio   P: portfolio view";
inline constexpr int kHomeThreeRowHelpExtraCols = 6;

inline bool home_use_three_row_help_layout(int term_cols)
{
    return term_cols < static_cast<int>(kHomeHelpMainRowWide.size()) + 1 +
                           kHomeThreeRowHelpExtraCols;
}
...
inline void render_home(AppState& app)
{
    ...
    int help_lines = 0;
    const bool home_three_row_help = home_use_three_row_help_layout(COLS);
    if (app.settings.show_help) {
        if (LINES >= 10)
            help_lines = 4;
        else if (LINES >= 8)
            help_lines = home_three_row_help ? 3 : 2;
    }
    ...
    auto print_help = [&](int y, std::string_view text) {
    ...
    if (help_lines >= 4) {
        ...
        if (app.tickers.search_mode) {
            attron(A_DIM);
            print_help(y0 + 2, "esc: exit search");
            print_help(y0 + 3, "q: quit   s: settings   ?: help");
            attroff(A_DIM);
        }
        else {
            attron(A_DIM);
            if (home_three_row_help) {
                print_help(y0 + 1, kHomeHelpMainRowNarrowTop);
                print_help(y0 + 2, kHomeHelpMainRowNarrowMiddle);
                print_help(y0 + 3, kHomeHelpMainRowNarrowBottom);
            }
            else {
    ...
    else if (help_lines >= 3) {
        const int y0 = std::max(0, LINES - 3);
        if (app.tickers.search_mode) {
            attron(A_DIM);
            print_help(y0 + 1, "esc: exit search");
            print_help(y0 + 2, "q: quit   s: settings   ?: help");
            attroff(A_DIM);
        }
        else {
            attron(A_DIM);
            print_help(y0 + 0, kHomeHelpMainRowNarrowTop);
            print_help(y0 + 1, kHomeHelpMainRowNarrowMiddle);
            print_help(y0 + 2, kHomeHelpMainRowNarrowBottom);
            attroff(A_DIM);
        }
    }
        ...
        if (app.tickers.search_mode) {
            attron(A_DIM);
            print_help(y0 + 0, "esc: exit search");
            print_help(y0 + 1, "q: quit   s: settings   ?: help");
            attroff(A_DIM);
        }
        else {
            attron(A_DIM);
